...
function GM:PlayerNoClip( pl, on )
	...
	if SinglePlayer() or (server_settings.Bool("SB_NoClip") and server_settings.Bool("sbox_noclip") and (pl.planet or AllowAdminNoclip(pl) or not server_settings.Bool("SB_PlanetNoClipOnly"))) then return true end
	return false
end
...
function GM:RemoveSBProps(world)
	for _, class in ipairs( self.affected ) do
		...
		for _, ent in ipairs( stuff ) do
			...
			elseif not world and not ent.planet then
				if not ent:IsPlayer() then
					local delay = (math.random(1, 500) / 100)
					ent:SetKeyValue("exploderadius","1")  
					ent:SetKeyValue("explodedamage","1")  
