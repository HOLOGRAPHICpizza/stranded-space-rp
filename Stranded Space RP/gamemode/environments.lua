...
function SB_Get_Environment_Info(num, astable)
	if environments[num] then
		if astable then
			return table.Copy(environments[num])
		else
			if environments[num].IsPlanet then
				return true, true, environments[num].ent:GetPos(),environments[num].radius, environments[num].gravity, environments[num].habitat, environments[num].air, environments[num].co2, environments[num].n, environments[num].atmosphere, environments[num].pressure, environments[num].ltemperature, environments[num].stemperature, environments[num].sunburn, environments[num].unstable
			else
...
function SB_OnEnvironment(pos, art,  extra, ignoredata)
	...
	-- Fixes some bugs when on a non-spacebuild map.
...
function SB_Terraform_Step(ent, num, lt, st, hab,atm)
	if ent:IsValid() and environments[num].tf and environments[num].tf == ent and ent.Active == 1 then
		...
		if not ok then
			timer.Simple(6, SB_Terraform_Step, ent, num, lt, st, hab,atm)
		end
	end
end
...
function SB_Terraform_UnStep(ent, num, other)
	if not other and not ent:IsValid() or (ent.Active == 0 and environments[num].tf and environments[num].tf == ent) then 
		...
		if not ok then
			timer.Simple(6, SB_Terraform_UnStep, ent, num, other)
		end
	end
end
...
function SB_RestorePlanetValues(num)
	if not Complete_Planet_Backup[num] then return false end
	local planet = Complete_Planet_Backup[num]
	Msg("Restoring planet values\n")
	PrintTable(planet)
	environments[num] = table.Copy(Complete_Planet_Backup[num])
end